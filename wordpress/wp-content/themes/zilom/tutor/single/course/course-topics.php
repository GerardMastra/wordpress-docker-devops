   ...
   <div class="tutor-single-course-segment single-course-topics-wrap">
      <div class="tutor-course-topics-header">
         <div class="tutor-course-topics-header-left">
            <h4 class="tutor-segment-title"><?php echo esc_html__('Topics for this course', 'zilom'); ?></h4>
         </div>
         <div class="tutor-course-topics-header-right">
				...
				if($tutor_lesson_count) {
					echo '<span>' . esc_html($tutor_lesson_count) . ' ' . esc_html__('Lessons', 'zilom') . '</span>';
				}
				...
				while ($topics->have_posts()){ $topics->the_post();
                 ...
                 <div class="tutor-course-topic tutor-topics-in-single-lesson <?php if($index == 1) echo "tutor-active"; ?>">
                     <div class="tutor-course-title <?php echo esc_attr($topic_summery ? 'has-summery' : ''); ?>">
	               	...
	               	<div class="tutor-course-lessons" style="<?php if($index > 1){echo 'display: none';} ?>">
								<?php
		                              ...
		                              <h5>
													<?php
														...
														if (has_post_thumbnail()){
															...
															$lesson_title .= "<i class='icon $lesson_icon'></i>";
														}
														...
														if ($is_enrolled || (get_post_meta($course_id, '_tutor_is_public_course', true)=='yes' && !tutor_utils()->is_course_purchasable($course_id))){
															...
															$lesson_title .= '<span class="lesson-meta">';
															...
															echo html_entity_decode($lesson_title);
														}else{
															$lesson_title .= "<a href='" . get_the_permalink() . "'> " . get_the_title() . " </a>";
															$lesson_title .= '<span class="lesson-meta">';
																$lesson_title .= $play_time ? "<span class='tutor-lesson-duration'>".tutor_utils()->get_optimized_duration($play_time)."</span>" : '';
																$lesson_access = get_post_meta(get_the_ID(), '_lesson_access', true);
																if($lesson_access != 'lesson_access_free'){
																	$lesson_title .= '<span class="lesson-preview-icon"><i class="tutor-icon-lock"></i> </span>';
																}
															$lesson_title .= '</span>';
